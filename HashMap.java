...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    /**
     * 使用树存储而不是使用链表存储的阈值。当添加一个元素使桶的节点数达到此值时就从列表结构转为树结构。
     * 该值必须大于2并且应该至少为8，以便与当移除节点时树收缩为普通链表时相匹配。
     */
    static final int TREEIFY_THRESHOLD = 8;

    /**
     * 在resize操作时判断是否进行非树化的阈值。该值应该小于TREEIFY_THRESHOLD
     * 并且至少为6，以匹配在移除操作时进行收缩检查。
     */
    static final int UNTREEIFY_THRESHOLD = 6;

    /**
     * 桶被树化的最少表容量。(否则如果一个桶里有太多节点时表将被扩容)
     * 至少应该是 4 * TREEIFY_THRESHOLD 来避免扩容和树化之间的阈值冲突。
     */
